    ...
    <div class="container">
        ...
        <div class="search-container mb-4">
            <form id="searchForm" method="GET" action="{{ route('search') }}">
                <div class="d-flex">
                    <input type="text" id="searchInput" name="q" class="form-control me-2" value="{{ $query }}" placeholder="Search forums, users, or tags..." />
                            ...
                            <div class="row">
                                <div class="col-md-4">
                                    ...
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" name="content_type[]" value="image" {{ in_array('image', $contentType ?? []) ? 'checked' : '' }}>
                                        <label class="form-check-label">With Images</label>
                                    </div>
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" name="content_type[]" value="video" {{ in_array('video', $contentType ?? []) ? 'checked' : '' }}>
                                        <label class="form-check-label">With Videos</label>
                                    </div>
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" name="content_type[]" value="links" {{ in_array('links', $contentType ?? []) ? 'checked' : '' }}>
                        ...
                        <div class="results-section mb-4">
                            ...
                            @foreach($users->take(3) as $user)
                                <div class="social-post">
                                    <div class="card-body">
                                        <div class="d-flex align-items-center">
                                            <img src="{{ $user->avatar_url ?? asset('assets/images/profile.png') }}" class="rounded-circle me-3" width="40" height="40" alt="Avatar">
                                            <div class="flex-grow-1">
                    ...
                    @if(isset($users) && $users->count() > 0)
                        @foreach($users as $user)
                            <div class="social-post">
                                <div class="card-body">
                                    <div class="d-flex align-items-center">
                                        <img src="{{ $user->avatar_url ?? asset('assets/images/profile.png') }}" class="rounded-circle me-3" width="50" height="50" alt="Avatar">
                                        <div class="flex-grow-1">
                                            <h6 class="mb-1 fw-bold">{{ $user->username }}</h6>
